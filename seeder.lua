...
theta = 0.46373398 --angle in radian (26.57 degree)
...
NOdebug = {debugMode = true, debugFiles=true, debugMesh = 'restart/'}
...
-- Seed point
  {
    ...
    geometry = {
      kind = 'canoND',
      object = {
        origin = { dx_eps, height/4.0+dx_eps+(length*math.sin(theta)), dx_eps },
      } --- object
    }
  },
  {
    ...
    geometry = {
      ...
      object = {
        ...
        vec = {
          {(length*math.cos(theta)) + 2*dx_eps, (length*math.sin(theta)) , 0.0},
  ...
  },
  {
    ...
    geometry = {
      ...
      object = {
        ...
        vec = {
          {(length*math.cos(theta))+2*dx_eps, (length*math.sin(theta))+ 2*dx_eps, 0.0},
          {0.0, 0.0, width+2.*dx_eps}
        }
      }
    }
    ...
    geometry = {
      ...
      object = {
        plane1={
          ...
          vec = {
            {0.0, 0.0, width+2*dx_eps},
--            {0.0, 2*height+2*dx_eps, 0.0}
            {0.0, height+2*dx_eps, 0.0}
          }
        },       
        plane2 = {
--          origin = {(length*math.cos(theta))+dx_eps, -dx_eps, -dx_eps},
        origin = {(length*math.cos(theta))+2*dx_eps, (length*math.sin(theta))+dx_eps, -dx_eps},
          vec = {
    ...
    geometry = {
      ...
      object = {
        plane1 = {
          ...
          vec = {
            {(length*math.cos(theta))+2*dx_eps, 0.0, 0.0},
            {0.0, 2*height+2*dx_eps, 0.0}
          }
        },  --- plane1
        plane2 = {
